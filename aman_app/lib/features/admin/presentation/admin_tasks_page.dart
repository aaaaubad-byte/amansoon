          ...
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                if (tasks.isEmpty) const Padding(padding: EdgeInsets.all(50), child: Center(child: Text('لا توجد مهام.'))),
                ...tasks.map((task) => Card(
                  margin: const EdgeInsets.only(bottom: 12),
                  child: ListTile(
                    leading: Icon(task.status == 'completed' ? Icons.check_circle : Icons.task_alt),
                    title: Text('الدورة ${task.cycleNumber} — ${task.taskAmount}'),
                    subtitle: Text('الاستحقاق: ${task.dueAt.toLocal().toString().split('.').first}\n${_statusLabel(task.status)}'),
                    isThreeLine: true,
