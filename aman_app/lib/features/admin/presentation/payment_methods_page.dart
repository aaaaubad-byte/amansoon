...
class _PaymentMethodsPageState extends State<PaymentMethodsPage> {
  ...
  Future<void> _add() async {
    final input = await showDialog<_PaymentInput>(
        context: context, builder: (_) => const _PaymentDialog());
    if (input == null) return;
    try {
      await _repository.create(
          name: input.name, type: input.type, accountDetails: input.details);
  ...
  Future<void> _edit(PaymentMethod method) async {
    final input = await showDialog<_PaymentInput>(
        context: context, builder: (_) => _PaymentDialog(method: method));
    if (input == null) return;
    try {
  ...
  Future<void> _toggle(PaymentMethod method) async {
    try {
      ...
      _notice(
          method.isActive ? 'تم تعطيل وسيلة الدفع.' : 'تم تفعيل وسيلة الدفع.');
      _refresh();
    } on PostgrestException catch (error) {
      _showError(_friendlyError(error));
    }
  ...
  String _friendlyError(PostgrestException error) => error.code == '42501'
      ? 'ليست لديك صلاحية لتنفيذ العملية.'
      : error.message;
              ...
              child: ListView(padding: const EdgeInsets.all(20), children: [
                ...
                if (methods.isEmpty)
                  const Padding(
                      ...
                      child: Center(child: Text('لا توجد وسائل دفع.'))),
                ...methods.map(
                  (method) => Card(
                    child: ListTile(
                      leading: Icon(
                      ...
                      subtitle: Text(
                        '${_typeLabel(method.type)}\n${method.accountDetails}\n'
                        '${method.isActive ? 'فعالة' : 'معطلة'}',
                      ),
                      isThreeLine: true,
                      trailing: Wrap(
                        children: [
...
class _PaymentInput {
  const _PaymentInput(
      {required this.name, required this.type, required this.details});
  final String name;
  final String type;
...
class _PaymentDialogState extends State<_PaymentDialog> {
  ...
  void initState() {
    ...
    _details = TextEditingController(text: widget.method?.accountDetails);
    _type = widget.method?.type ?? 'wallet';
  }
  ...
  Widget build(BuildContext context) => AlertDialog(
          title: Text(
              widget.method == null ? 'إضافة وسيلة دفع' : 'تعديل وسيلة الدفع'),
          content: Form(
              key: _formKey,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(value: 'exchange', child: Text('صرافة'))
                    ],
                    onChanged: (value) {
                ...
                TextFormField(
                    ...
                    maxLines: 2,
                    decoration:
                        const InputDecoration(labelText: 'بيانات الحساب'),
